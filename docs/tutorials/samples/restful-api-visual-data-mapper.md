...
function transformCustomer(BackendCustomer backend) returns CustomerProfile => {
    ...
    shippingAddress: backend.shipping_address is BackendAddress
        ? transformAddress(backend.shipping_address)
        : (),
...
function transformOrder(BackendOrder backend, map products) returns CustomerOrder => {
...
- [Data Service with bal persist](data-service-bal-persist.md) -- Build a full CRUD data service
- [Event-Driven Microservices](event-driven-microservices-kafka.md) -- Build microservices with Kafka
- [Visual Data Mapper Guide](../../develop/transform/visual-data-mapper.md) -- Learn more about the data mapper
